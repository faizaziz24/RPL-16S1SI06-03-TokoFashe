	...
	<footer class="bg6 p-t-45 p-b-43 p-l-45 p-r-45">
		...
		<div class="t-center p-l-15 p-r-15">
			<div class="t-center s-text8 p-t-20">
				Copyright © 2018 All rights reserved. | This template is made with <i class="fa fa-heart-o" aria-hidden="true"></i> by fashe.
			</div>
		</div>
	</footer>
		...
		$(document).ready(function(){
			...
			$('#cmbJenis').on('change', function(){
				...
				if (jenisID) {
					$.ajax({
						type 	: 'POST',
						url  	: 'pages/custom/customData.php',
						data 	: 'kd_jenis='+jenisID,
						dataType: 'html',
						success	: function(response){
							$('#cmbKategori').html(response);
						}
					});
				}else{
					$('#cmbKategori').html('<option value="">Pilih Jenis Barang Terlebih Dahulu</option>');
				}
			});

			$('#cmbKategori').on('change', function(){
				var katID = $(this).val();
				if (katID) {
					$.ajax({
						type 	: 'POST',
						url  	: 'pages/custom/customData.php',
						data 	: 'kd_kategori='+katID,
						dataType: 'html',
						success	: function(response){
							$('#cmbBahan').html(response);
						}
					});
				}else{
					$('#cmbBahan').html('<option value="">Pilih Kategori Barang Terlebih Dahulu</option>');
				}
			});
			...
			$('#cmbBahan').on('change', function(){
				...
				if (lthID) {
					$.ajax({
						...
						dataType: 'html',
						success	: function(response){
							$('#cmbUkuran').html(response);
						}
					});
				}else{
					$('#cmbUkuran').html('<option value="">Pilih Bahan Barang Terlebih Dahulu</option>');
				}
			});

			$('#cmbUkuran').on('change', function(){
				var sizeID = $(this).val();
				if (sizeID) {
					$.ajax({
						type 	: 'POST',
						url  	: 'pages/custom/customData.php',
						data 	: 'kd_ukuran='+sizeID,
						dataType: 'html',
						success	: function(response){
							$('#cmbWaktu').html(response);
						}
					});
				}else{
					$('#cmbWaktu').html('<option value="">Pilih Ukuran Barang Terlebih Dahulu</option>');
				}
			});
		});
	</script>
